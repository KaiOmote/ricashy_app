...
- [x] **Milestone 1: Project Setup & Core Architecture**
- [x] **Milestone 2: Transaction Management**
...
*   [x] **Task 1.1:** Initialize the Flutter project and configure it for desktop development (Windows, macOS, Linux).
    *   **Verification:** Run `flutter run` on a desktop target to ensure the default application builds and launches successfully.
*   [x] **Task 1.2:** Add initial dependencies to `pubspec.yaml`: `flutter_riverpod`, `drift` (for the database), `path_provider`, `intl`, and `fl_chart`.
    *   **Verification:** Run `flutter pub get` and then `flutter run` to confirm that the project still builds and runs without errors.
*   [x] **Task 1.3:** Create the project directory structure as defined in `GEMINI.md`.
    *   **Verification:** Check that the directory structure is in place and run `flutter run`.
*   [x] **Task 1.4:** Implement the main application shell with a basic desktop layout (e.g., a side navigation rail and a main content area).
    *   **Verification:** Run `flutter run` to see the new layout and navigate between placeholder pages.
*   [x] **Task 1.5:** Set up the `drift` database with initial tables for `transactions` and `categories`.
    *   **Verification:** Write a unit test to open the database and verify table creation. Run the app to ensure it doesn't crash.
*   [x] **Task 1.6:** Implement the Riverpod providers for accessing the database.
...
*   [x] **Task 2.1:** Build the UI form for adding and editing transactions (income and expenses).
    *   **Verification:** Run the app and open the form. Test the UI elements.
*   [x] **Task 2.2:** Implement the logic to save, update, and delete transactions in the database.
